...
title.textContent = `Resultados da busca: "${search}"`;
...
const createContentCard = ({name, category, url, price}) => {
    // adicionar  detalhe do produto com ID
    return `<img class="category__img" src="${url}" alt="${category}" onerror="imgErrorHTML(this)">
            <div class="category__text">
                ...
                <p class="category__detail">Ver produto <i class="fa-solid fa-arrow-right-to-bracket"></i></p>
            </div>`;
};
...
const noCoincidence = () => {
    productsBox.innerHTML = `<div class="no-coincidence">
                                ...
                                <p>Nenhum resultado foi encontrado para corresponder à sua pesquisa.</p>
                             </div>`;
}
